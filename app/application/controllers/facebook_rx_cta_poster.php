...
class facebook_rx_cta_poster extends Home
{
    ...
    public function cta_post_list_data()
    {
        ...
        for($i=0;$i<count($info);$i++) 
        {
            ...
            if($posting_status=='2')
            $post_url = "<a target='_BLANK' href='".$info[$i]['post_url']."'><span class='label label-primary'><i class='fa fa-hand-o-right'></i> Visit</span></a>";
            else $post_url = '<i class="fa fa-remove red" title="This post is not published yet."></i>';
            $info[$i]['visit_post'] =  $post_url;    

            $info[$i]['delete'] =  "<a title='Delete this post from our database' id='".$info[$i]['id']."' class='delete btn-sm btn btn-danger'><i class='fa fa-remove'></i> Delete</a>"; 

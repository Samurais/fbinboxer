...
class facebook_rx_account_import extends Home
{
    ...
    public function ajax_delete_group_action()
    {
        ...
        echo "<div class='alert alert-success text-center'>Your group has been deleted successfully.</div>";
    }
    ...
    public function ajax_delete_page_action()
    {
        ...
        $data = array('deleted' => '1');
        $this->basic->update_data('facebook_rx_fb_page_info',array('id'=>$table_id),$data);
        echo "<div class='alert alert-success text-center'>Your page has been deleted successfully.</div>";
    }
    ...
    public function ajax_delete_account_action()
    {
        ...
        //******************************//
        $this->basic->update_data('facebook_rx_fb_user_info',array('id'=>$table_id),$data);
        $this->basic->update_data('facebook_rx_fb_page_info',array('facebook_rx_fb_user_info_id'=>$table_id),$data);
        $this->basic->update_data('facebook_rx_fb_group_info',array('facebook_rx_fb_user_info_id'=>$table_id),$data);
    ...
    public function send_user_roll_access()
    {
        if($_POST)
        {
            ...
            if(isset($response['success']) && $response['success'] == 1)
                echo "<br/>
            <div class='well'><h4 class='text-center red'>Please log in & check your facebook profile page notifications, to accept our invitation</h4></div>
            <div class='alert alert-danger text-center'>
                        <h4 style='line-height:25px'>A request has been sent to your facebook account. Please login to your facebook account, confirm the app request and click below button.<br/><br/>DO NOT CLICK THIS UNTIL CONFIRMED</h4>
                        <br/>
                        <button class='btn btn-default btn-lg' id='fb_confirm'><b>I've Confirmed App Request in Facebook</b></button>
                    </div>";
                ...
                echo "<br/><div class='alert alert-danger text-center'>
                        <p><i class='fa fa-remove'></i> Something went wrong, please try with correct information.</p>
                    </div>";
        }
    }
    ...
    public function ajax_get_login_button()
    {
        ...
        if(isset($fb_login_button))
        {
            ...
            echo "<br/><div class='alert alert-danger text-center'><p>Something went wrong, Please try again with proper information</p></div>";
    }
    ...
    public function redirect_custer_link()
    {
        ...
        else 
        {
            ...
            if($status=="2") 
            {
                $this->session->set_userdata('limit_cross', 'Sorry, your account import limit has been exceeded.');
            ...
            if(isset($permission['data']['scopes']))
            {
                ...
                if(empty($permission_checking))
                {
                    ...
                    $text = "Sorry, you didn't confirm the request yet. Please login to your FB account and accept the request. For more <a href='".$documentation_link."'>Visit Here.</a>";

...
class fb_msg_manager extends Home
{
    ...
    public function get_unread_message()
    {
        ...
        if(!empty($last_sync_time))
        {
            ...
            $str .= '<h3 class="red refresh_button_holder"><div class="well text-center clearfix">Last update time : '.date("Y-m-d H:i:s",strtotime($last_sync_time)).'<button id="refresh_button" class="btn btn-success pull-right"><i class="fa fa-refresh"></i> Refresh Data</button></div></h3>';
        }



        $str .= '<div class="well text-center"><h3 class="header_title">All unread messages</h3></div>
                <script>
    ...
    public function get_pages_conversation()
    {
        ...
        else
        {
            ...
            {
                $str .= '<div class="well text-center"><h3 class="header_title"><img class="img-thumbnail" src="'.$value['page_profile'].'" alt="image"> '.$value['page_name'].' (Last 20 conversation)</h3></div>';
                $where = array();
                $where['where'] = array(
                    'user_id' => $this->user_id,
                    'facebook_rx_fb_user_info_id' => $this->session->userdata('facebook_rx_fb_user_info'),
                ...
                {
                    $str .= '<div class="alert alert-danger text-center"><h3>No data to show !</h3></div><br/>';
                }
                else
                {
                    $str .= '<div><script>
